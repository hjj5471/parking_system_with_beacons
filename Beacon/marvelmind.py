...
class MarvelmindHedge(Thread):
    ...
    def print_position(self):
        if (isinstance(self.position()[1], int)):
            print(
                "Hedge {:d}: X: {:d} m, Y: {:d} m, Z: {:d} m, Angle: {:d} at time T: {:.2f}".format(self.position()[0],
                                                                                                    self.position()[1],
                                                                                                    self.position()[2],
                                                                                                    self.position()[3],
                                                                                                    self.position()[4],
            ...
            print(
                "Hedge {:d}: X: {:.3f}, Y: {:.3f}, Z: {:.3f}, Angle: {:d} at time T: {:.2f}".format(self.position()[0],
                                                                                                    self.position()[1],
                                                                                                    self.position()[2],
                                                                                                    self.position()[3],
                                                                                                    self.position()[4],
    ...
    def print_distances(self):
        self.distancesUpdated = False
        print("Distances: B{:d}:{:.3f}, B{:d}:{:.3f}, B{:d}:{:.3f}, B{:d}:{:.3f}   at time T: {:.2f}".format(
            self.distances()[1], self.distances()[2], self.distances()[3], self.distances()[4], self.distances()[5],
            self.distances()[6], self.distances()[7], self.distances()[8], self.distances()[9] / 1000.0))
                            ...
                            try:
                                if (len(bufferList) > pktHdrOffset + 4 + msgLen + 2):
                                    ...
                                    if (isCmMessageDetected):
                                        usnTimestamp, usnX, usnY, usnZ, usnAdr, usnAngle, usnCRC16 = struct.unpack_from(
                                            '<LhhhxBhxxH', strbuf, pktHdrOffset + 5)
                                        usnX = usnX / 100.0
                                        usnY = usnY / 100.0
                                        usnZ = usnZ / 100.0
                                        ...
                                        usnTimestamp, usnX, usnY, usnZ, usnAdr, usnAngle, usnCRC16 = struct.unpack_from(
                                            '<LlllxBhxxH', strbuf, pktHdrOffset + 5)
                                        usnX = usnX / 1000.0
                                        usnY = usnY / 1000.0
                                        usnZ = usnZ / 1000.0
                                        usnAngle = 0b0000111111111111 & usnAngle
                                        ...
                                        HedgeAdr, b1, b1d, b2, b2d, b3, b3d, b4, b4d, timestamp, usnCRC16 = struct.unpack_from(
                                            '<BBlxBlxBlxBlxLxxxH', strbuf, pktHdrOffset + 5)
                                    ...
                                    if CRC_calc == usnCRC16:
                                        if (isMmMessageDetected or isCmMessageDetected):
                                            ...
                                            value = [HedgeAdr, b1, b1d / 1000.0, b2, b2d / 1000.0, b3, b3d / 1000.0, b4,
                                                     b4d / 1000.0, timestamp]
